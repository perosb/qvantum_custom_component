...
class TestQvantumSelectEntity:
    ...
    async def test_async_select_option(self, mock_coordinator, mock_device):
        ...
        # Mock the API call
        mock_coordinator.async_set_smartcontrol = AsyncMock()

        # Test selecting Off
        await entity.async_select_option("off")
        mock_coordinator.async_set_smartcontrol.assert_called_with(
            "test_device_123", -1, -1
        )
    ...
    async def test_async_select_option_mode_synchronization_no_response(
        ...
        # Mock None response
        mock_coordinator.async_set_smartcontrol = AsyncMock(return_value=None)
...
class TestQvantumSelectEntityOperationSensor:
    ...
    async def test_async_select_option(self, mock_coordinator, mock_device):
        ...
        for mode in SensorMode:
            opt = str(mode.value)
            mock_coordinator.async_write_metric.reset_mock()
