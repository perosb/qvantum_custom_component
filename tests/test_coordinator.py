...
class TestQvantumDataUpdateCoordinator:
    ...
    async def test_async_update_data_http_includes_tap_stop(self, mock_super_init):
        ...
        coordinator = QvantumDataUpdateCoordinator(mock_hass, mock_config_entry, client=make_client_mock())
...
class TestDeviceLookupWhenHttpDown:
    ...
    async def test_async_update_data_api_connection_error_raises_update_failed(
        ...
        # Reproduce the mixed scenario where metrics succeed but settings encounter 503
        mock_api.get_metrics = AsyncMock(
        ...
        assert "Error communicating with API for device test_device" in caplog.text
