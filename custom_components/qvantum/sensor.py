...
async def async_setup_entry(
    ...
    for metric in metrics.get("metrics"):
        if metric.get("unit") == UnitOfTemperature.CELSIUS:
            sensors.append(QvantumTemperatureEntity(coordinator, metric.get("name"), metric.get("name").replace("_", " ").lower(), device))
        elif metric.get("unit") == UnitOfEnergy.KILO_WATT_HOUR:
            sensors.append(QvantumEnergyEntity(coordinator, metric.get("name"), metric.get("name").replace("_", " ").lower(), device))
        else:
            sensors.append(QvantumBaseEntity(coordinator, metric.get("name"), metric.get("name").replace("_", " ").lower(), device))

    sensors.append(QvantumTotalEnergyEntity(coordinator, "totalenergy", "total energy", device))
    sensors.append(QvantumConnectivityEntity(coordinator, "timestamp", "timestamp", device))
    sensors.append(QvantumTimerEntity(coordinator, "extra_tap_water_stop", "extra tap water stop", device))
    sensors.append(QvantumTimerEntity(coordinator, "ventilation_boost_stop", "ventilation boost stop", device))
    sensors.append(QvantumConnectivityEntity(coordinator, "disconnect_reason", "disconnect reason", device))
...
class QvantumEnergyEntity(QvantumBaseEntity):
    ...
    def available(self):
        """Check if data is available."""
        return super().available and self.coordinator.data.get("metrics").get(self._metric_key) > 0
